...
import { mockAuthState, mockCandidates } from './setup';
import { renderWithAuth, wrapWithAuth } from './testUtils';
...
describe('MatchDetailPage', () => {
  it('renders the match summary and estado del evento on the datos tab by default', () => {
    renderWithAuth(<MatchDetailPage match={baseMatch} unlinkedBookings={[]} />);
  ...
  it('shows reservation actions when the match has no court yet', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, courtName: null }} unlinkedBookings={[]} />);
  ...
  it('shows a discreet "Sede pendiente de confirmación" message when the venue is still undecided, not an error', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, clubName: null, venueType: 'TO_BE_DEFINED', clubId: null }} unlinkedBookings={[]} />);
  ...
  it('shows Completo once the backend reports the match as FULL', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, status: 'FULL', maxPlayers: '1' }} unlinkedBookings={[]} />);
  ...
  it('shows a banner when the current user has a pending or accepted invitation to this match', () => {
    const { rerender } = renderWithAuth(<MatchDetailPage match={baseMatch} unlinkedBookings={[]} myInvitationStatus="PENDING" />);
    expect(screen.getByText(/invitación pendiente/i)).toBeTruthy();

    rerender(wrapWithAuth(<MatchDetailPage match={baseMatch} unlinkedBookings={[]} myInvitationStatus="ACCEPTED" />));
    expect(screen.getByText(/invitación aceptada/i)).toBeTruthy();

    rerender(wrapWithAuth(<MatchDetailPage match={baseMatch} unlinkedBookings={[]} myInvitationStatus={null} />));
  ...
  it('resumen tab lets the organizer cancel the match', () => {
    ...
    renderWithAuth(<MatchDetailPage match={baseMatch} unlinkedBookings={[]} onCancelMatch={onCancelMatch} />);
  ...
  it('a non-organizer (confirmed participant or invited guest) never sees management actions: no Gestión del partido card, no schedule-edit form', () => {
    const onCancelMatch = vi.fn();
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, isOrganizer: false }} unlinkedBookings={[]} onCancelMatch={onCancelMatch} />);
  ...
  it('lets the organizer pick Otro and type a free-text venue name', async () => {
    const onEditSchedule = vi.fn().mockResolvedValue(undefined);
    renderWithAuth(<MatchDetailPage match={baseMatch} unlinkedBookings={[]} onEditSchedule={onEditSchedule} />);
  ...
  it('does not offer Cancelar partido once the match is already cancelled', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, status: 'CANCELLED' }} unlinkedBookings={[]} />);
  ...
  it('shows the availability window and a pending message when no exact time was confirmed yet', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, startsAt: null, endsAt: null }} unlinkedBookings={[]} />);
  ...
  it('editing the schedule sends the updated day, franja and exact time, and recalculates the display', async () => {
    const onEditSchedule = vi.fn().mockResolvedValue(undefined);
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, startsAt: null, endsAt: null }} unlinkedBookings={[]} onEditSchedule={onEditSchedule} />);
  ...
  it.each(['IN_PROGRESS', 'EXPIRED', 'COMPLETED'] as const)(
    ...
    (status) => {
      renderWithAuth(<MatchDetailPage match={{ ...baseMatch, status }} unlinkedBookings={[]} />);
  ...
  it('shows the IN_PROGRESS read-only message', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, status: 'IN_PROGRESS' }} unlinkedBookings={[]} />);
  ...
  it('shows the EXPIRED read-only message', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, status: 'EXPIRED' }} unlinkedBookings={[]} />);
  ...
  it('shows the COMPLETED read-only message', () => {
    renderWithAuth(<MatchDetailPage match={{ ...baseMatch, status: 'COMPLETED' }} unlinkedBookings={[]} />);
